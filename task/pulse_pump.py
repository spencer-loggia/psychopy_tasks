...
def parse_args():
    ...
    parser.add_argument("--pump_pin", type=int, default=None, help="DAQC2 DOUT bit for the pump, 0-7")
...
def main():
    ...
    duration_s = max(0.0, float(_get("duration", 10.0)))
    ...
    try:
        outputs.open()
        output_on, output_off = outputs.bind_bit(int(_get("pump_pin", 0)))
    ...
    try:
        output_on()
        print(
            f"Priming pump on DAQC2 address {int(_get('daq_address', 0))} DOUT{int(_get('pump_pin', 0))} "
